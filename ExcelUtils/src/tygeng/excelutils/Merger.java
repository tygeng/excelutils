...
public class Merger {
	...
	public Merger(Workbook target, Logger log)
		...
		try {
			...
			for (int i = 0; i < numSheets; i++) {
				...
				if(Utils.getNonemptyRowSince(currentSheet, endRow)>0) {
					log.s("#### Sheet \""+currentSheet.getSheetName()+"\" may contain extra rows that is ill formated. Skipped.");
					continue;
				}
...
//				 + currentSheet.getSheetName());
				...
				if (targetSheetHeader == null) {
					...
					boolean[] isDate = new boolean[headerSize];
					...
					for (int j = 0; j < headerSize; j++) {
						...
						if (r2State.contains("Date")
								|| r4State.contains("Date")) {
							isDate[j] = true;
						}
